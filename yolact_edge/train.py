...
parser.add_argument('--config', default='my_custom_dataset', type=str,
                    help='The config object to use.')
parser.add_argument('--save_interval', default=10000, type=int,
                    help='The number of iterations between saving the model.')
parser.add_argument('--validation_size', default=800, type=int,
                    help='The number of images to use for validation.')
parser.add_argument('--validation_epoch', default=2, type=int,
                    help='Output validation information every n iterations. If -1, do no validation.')
# Added arguments for epochs and early stop
...
def train(rank, args=args):
    ...
    if cfg.dataset.name == 'YouTube VIS':
        ...
        val_dataset = None  # No validation set provided for FlyingChairs in this example.
    ...
    # Use args.epochs if nonzero, else fallback to max_iter-based epochs
    num_epochs = args.epochs if args.epochs > 0 else math.ceil(cfg.max_iter / epoch_size)
...
""".format(
            ...
            if args.validation_epoch > 0 and epoch > 0 and (epoch % args.validation_epoch == 0):
                # Determine an appropriate collate function for the validation loader
                val_collate = collate_fn_youtube_vis if cfg.dataset.name == 'YouTube VIS' else detection_collate
                if val_dataset is not None:
                    ...
                    if rank == 0:
                        ...
                        current_val_loss = compute_validation_loss(net, val_loader, criterion)
                        logger.info("Epoch {}: Validation Loss = {:.3f}".format(epoch, current_val_loss))
...
def compute_validation_loss(net, data_loader, criterion):
    ...
    with torch.no_grad():
        ...
        for datum in data_loader:
            images, targets, masks, num_crowds = prepare_data(datum)
            # Pass extras to the network as required
            extras = {"backbone": "full", "interrupt": False, "moving_statistics": {"aligned_feats": []}}
            net_outs = net(images, extras=extras)
            # IMPORTANT: extract "pred_outs" as done during training.
            out = net_outs["pred_outs"]
            _losses = criterion(out, targets, masks, num_crowds)
            
            for k, v in _losses.items():
                v = v.mean().item()  # Mean here because of DataParallel
        ...
        # Compute average loss for each component
        for k in losses:
            losses[k] /= iterations
