...
class Annotation2D:
    ...
    def exportCOCO(self, image_id, file_name, width, height):
        """
        Convert the loaded instance annotations into COCO-format.
        ...
        Returns:
            A dictionary with keys 'images', 'annotations', and 'categories' following COCO format.
        """
        # Ensure that the instance data has been loaded.
        if self.semanticId is None or self.instanceId is None:
            raise Exception("Instance data not loaded. Please call loadInstance() first.")
        
        # Compute a global identifier for each pixel.
        global_ids = local2global(self.semanticId, self.instanceId)
        unique_ids = np.unique(global_ids)
        annotations = []
        annotation_id_counter = 1  # Unique annotation id
        ...
        for uid in unique_ids:
            ...
            mask = (global_ids == uid).astype(np.uint8)
            
            # Compute the area as the sum of pixels.
            area = int(np.sum(mask))
            if area == 0:
                continue
            
            # Compute bounding box: [xmin, ymin, width, height]
            ys, xs = np.where(mask)
            xmin, xmax = int(np.min(xs)), int(np.max(xs))
            ymin, ymax = int(np.min(ys)), int(np.max(ys))
            bbox = [xmin, ymin, xmax - xmin, ymax - ymin]
            
            # Extract segmentation polygons using find_contours.
            contours = measure.find_contours(mask, 0.5)
            segmentation = []
            for contour in contours:
                # Each contour is an array of shape (num_points, 2) in (row, col) order.
                # Flip coordinates to (x, y) format as required by COCO.
                contour = np.flip(contour, axis=1)
                # Flatten the coordinates and convert to a list of floats.
                poly = contour.ravel().tolist()
                if len(poly) < 5:
                    continue
            ...
            if len(segmentation[0]) < 5:
                continue

            # Retrieve the semantic and instance IDs for this instance.
            semantic, instance = global2local(uid)
            
            ann_dict = {
                "id": annotation_id_counter,
                "image_id": image_id,
                "category_id": int(semantic),  # Using the semantic ID as the category id.
        ...
        # Build a simple category list based on the unique semantic ids (excluding background).
        semantic_ids = np.unique(self.semanticId)
        categories = []
        for sem in semantic_ids:
            if sem == 0:
                continue
            cat_dict = {
                "id": int(sem),
                "name": "category_{}".format(sem),
                "supercategory": "none"
            }
    ...
    def create_coco_annotations(instance_dir, output_json):
        """
        Process every instance label image in a specified directory, convert each to the COCO format,
        and write a combined COCO JSON file.

        Parameters:
            instance_dir (str): Path to the directory containing instance label images (PNG files).
            output_json (str): Path where the combined COCO JSON file will be written.

        Returns:
            None. The function writes out the JSON file to output_json.
        """
        # Initialize lists to store image metadata, annotations, and categories.
        coco_images = []
        coco_annotations = []
        categories_dict = {}  # Merge categories by their id.
        global_annotation_id = 1
        global_image_id = 1

        # Get a sorted list of instance label PNG files.
        instance_files = sorted(glob.glob(os.path.join(instance_dir, "*.png")))
        #instance_files = instance_files[:10]
        if not instance_files:
            raise ValueError("No PNG files found in directory: " + instance_dir)

        for instance_file in instance_files:
            # Load the image and determine its dimensions.
            instance_img_array = io.imread(instance_file)
            if len(instance_img_array.shape) == 2:
                height, width = instance_img_array.shape
            else:
                height, width, _ = instance_img_array.shape
            
            # Create an Annotation2D object and load the instance image.
            ann2d = Annotation2D()
            ann2d.loadInstance(instance_file, toImg=False)
            file_name = os.path.basename(instance_file)
            
            # Export COCO-format annotations for this image.
            coco_single = ann2d.exportCOCO(
            ...
            # Update annotation ids to be globally unique.
            for ann in coco_single["annotations"]:
                ann["id"] = global_annotation_id
                global_annotation_id += 1
                coco_annotations.append(ann)
            
            # Add image info.
            for img in coco_single["images"]:
                coco_images.append(img)
            
            # Merge the categories (avoid duplicates by using the semantic id as key).
            for cat in coco_single["categories"]:
                cat_id = cat["id"]
        ...
        # Write the final COCO dictionary to the specified JSON file.
        with open(output_json, "w") as f:
            json.dump(final_coco, f, indent=4)
        
        print("COCO JSON file created with {} images and {} annotations.".format(
...
def main(instance_dir, output_json):
    # Example usage of create_coco_annotations
    Annotation2D.create_coco_annotations(instance_dir, output_json)
    

# a dummy example
if __name__ == "__main__":
    # provide the directory containing instance label images and the output JSON file path
    instance_dir = '/path/to/instance/labels'
    output_json = '/path/to/output/instanceDict.json'
    main()
